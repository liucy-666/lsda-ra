...
foreach ($job in $jobs) {
    ...
    $tag = 'a_retry1'
    $stdout = Join-Path $logs ("retry_" + $job.Rater + '.out.log')
    $stderr = Join-Path $logs ("retry_" + $job.Rater + '.err.log')
    $arguments = @(
    ...
    Set-Content -LiteralPath (Join-Path $logs ("retry_" + $job.Rater + '.pid')) `
