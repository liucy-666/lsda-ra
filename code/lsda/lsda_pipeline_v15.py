...
GUIDANCE = 4.5
SIZE = 1024
# v1.2: entity owners are dilated by this many image pixels so the entity expert
# owns the native-contour band and can fully fill/erase the previous shape.
DILATE_PX = int(os.environ.get("LSDA_DILATE_PX", "24"))
# v1.4: SAM used only as a locator -> each owner mask is reduced to its padded bounding
# rectangle; the entity expert regenerates object + inner background inside the rectangle.
RECT_PAD = int(os.environ.get("LSDA_RECT_PAD", "0"))
# v1.5: remove the hard rectangular seam (v1.4) by making the commit alpha cross-fade.
#   "mask"  : commit the SAM mask (optionally dilated), feathered; rectangle only sets the crop.
#   "alpha" : commit the padded rectangle with a feathered alpha so the expert's regenerated
#             background cross-fades into the native background instead of stepping at the edge.
FEATHER_PX = int(os.environ.get("LSDA_FEATHER_PX", "12"))
COMMIT_MODE = os.environ.get("LSDA_COMMIT", "mask")
...
def _dilate_mask(mask: np.ndarray, k: int) -> np.ndarray:
    ...
    m = m.filter(ImageFilter.MaxFilter(2 * k + 1))
    return np.asarray(m) > 127


def _feather_mask(mask: np.ndarray, k: int) -> np.ndarray:
    """Image-space soft alpha: Gaussian ramp of radius k (keeps the boundary seamless)."""
    if k <= 0:
        return mask.astype(np.float32)
    m = Image.fromarray((mask.astype(np.uint8) * 255)).filter(ImageFilter.GaussianBlur(k))
    return np.asarray(m).astype(np.float32) / 255.0


def build_commit_masks(owners_image: list[np.ndarray], mode: str, rect_pad: int) -> list[np.ndarray]:
    """Return the per-entity commit masks for the chosen v1.5 variant."""
    if mode == "alpha":
        return rectangularize_owners(owners_image, rect_pad)
    return owners_image  # "mask": keep SAM contours; rect is only used for the crop


def image_masks_to_latent(owners: list[np.ndarray], latent: torch.Tensor):
    # v1.5 = v1.2 soft boundary, plus image-space feathering so the commit alpha
    # cross-fades at the region boundary instead of stepping (removes the straight seam).
    tensors = []
    for owner in owners:
        binary = owner.astype(bool)
        grown = _dilate_mask(binary, DILATE_PX) if DILATE_PX > 0 else binary
        soft = _feather_mask(grown, FEATHER_PX) if FEATHER_PX > 0 else grown.astype(np.float32)
        value = torch.from_numpy(soft.astype(np.float32))[None, None].to(latent.device)
        value = F.interpolate(value, size=latent.shape[-2:], mode="area")
        tensors.append(value)
    stack = torch.stack(tensors)
    total = stack.sum(0)
    if bool((total > 1.0).any()):
        stack = stack / total.clamp(min=1e-6)
        total = stack.sum(0)
        tensors = [stack[i] for i in range(stack.shape[0])]
    background = (1.0 - total).clamp(0.0, 1.0)
    return tensors, background


def rectangularize_owners(owners: list[np.ndarray], pad: int = RECT_PAD) -> list[np.ndarray]:
    """Reduce each owner mask to its padded bounding rectangle (SAM as locator only)."""
    rects = []
    for mask in owners:
        ys, xs = np.where(mask)
        if len(xs) == 0:
            rects.append(mask.copy())
            continue
...
def owner_bounds(owner: torch.Tensor, patch_size: int, margin: int = 8):
...
def main():
    ...
    owners_image = build_commit_masks(owners_image, COMMIT_MODE, RECT_PAD)
