...
def image_masks_to_latent(owners: list[np.ndarray], latent: torch.Tensor):
    # v1.2 = v1.1 soft boundary + mask dilation.
    # Dilation lets each entity expert own the native-contour band (so it can fully
    # fill or erase the previous shape); area pooling keeps the new outer edge soft.
    tensors = []
    for owner in owners:
        grown = _dilate_mask(owner, DILATE_PX)
        value = torch.from_numpy(grown.astype(np.float32))[None, None].to(latent.device)
...
def foreground_split_masks(image: Image.Image, entities: list[dict], threshold: float = 0.18):
    """v1.3 no-SAM owner masks: color foreground + vertical split at foreground centroid."""
    threshold = float(os.environ.get("LSDA_FG_THRESHOLD", threshold))
    ...
    foreground = np.linalg.norm(arr - background_color[None, None], axis=-1) > threshold
    ys, xs = np.where(foreground)
    x_mid = float(np.median(xs)) if len(xs) else 0.5 * width
    owners = []
    for index, entity in enumerate(entities):
        x_center = expected_x(entity.get("position", ""), index, len(entities))
        side = foreground.copy()
        if x_center < 0.5:
            side[:, int(round(x_mid)):] = False
        else:
            side[:, :int(round(x_mid))] = False
        seed_point = (int(round(x_center * width)), int(round(0.5 * height)))
        component = seeded_component(side, seed_point)
        if component.sum() == 0:
            component = np.zeros_like(foreground, dtype=bool)
            if x_center < 0.5:
                component[:, :int(round(x_mid))] = True
            else:
                component[:, int(round(x_mid)):] = True
        owners.append(fill_holes(component))
    candidates = []
    for mask in owners:
        _, bbox, centroid, _, _ = mask_geometry(mask)
        candidates.append(Candidate(mask, (0, 0), 1.0, 1.0, float(mask.mean()), bbox, centroid))
    clean_owners, _, diagnostics = clean_and_partition(candidates)
    diagnostics["method"] = "no_sam_foreground_split"
    diagnostics["threshold"] = threshold
    diagnostics["x_mid"] = x_mid
    return clean_owners, diagnostics
...
def main():
    ...
    parser.add_argument("--sam-model-dir", type=Path, required=False, default=None)
    parser.add_argument(
        "--helpers-dir",
        type=Path,
        required=True,
    ...
    checks = [("model", args.model_dir), ("helper modules", args.helpers_dir)]
    if args.sam_model_dir is not None:
        checks.append(("SAM model", args.sam_model_dir))
    ...
    if args.reuse_segmentation and all(
    ...
    elif args.sam_model_dir is not None:
        ...
        # v1.3 no-SAM: color foreground + vertical split at foreground centroid.
        owners_image, segmentation_diagnostics = foreground_split_masks(ss, config["entities"])
        save_segmentation(ss, owners_image, segmentation_dir, config["entities"], segmentation_diagnostics)
