...
def image_masks_to_latent(owners: list[np.ndarray], latent: torch.Tensor):
    # v1.1 soft boundary: fractional coverage via area pooling instead of nearest one-hot.
    # Boundary latent cells get partial entity weight and partial background weight, so a
    # thin/oblique object edge is shared rather than snapped to background (native scaffold).
    tensors = []
    for owner in owners:
